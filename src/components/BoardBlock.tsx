      ...
      <Droppable droppableId={column.id}>
        {(provided, snapshot) => (
          <div
            ...
            className={`flex-1 overflow-y-auto min-h-[100px] space-y-2 mb-2 ${
                ...
                <Draggable key={task.id} draggableId={task.id} index={index}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`bg-white rounded-md border p-3 mb-2 ${
...
const TaskDetailDialog = ({ 
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogPortal>
        <DialogOverlay className="fixed inset-0 z-50 bg-black/80" onClick={onClose} />
...
const BoardBlock = ({
  ...
  useEffect(() => {
    ...
    if (block && (!block.content || block.content === '')) {
      console.log("Creating new board with default data");
      ...
      try {
        ...
        console.log("Parsing existing board data");
        ...
        // Update the block content to ensure it's properly formatted
        onUpdate({
          ...block,
    ...
    if (block && block.content) {
      try {
        ...
        if (currentContent !== block.content) {
          console.log("Block content changed, updating board data");
          const parsedData = parseBoardData();
  ...
  // Add a cleanup effect to ensure tasks are saved when component unmounts
  useEffect(() => {
    return () => {
      console.log("BoardBlock unmounting, saving final state");
      if (block && onUpdate) {
        onUpdate({
          ...block,
  ...
  // Task deletion functionality has been removed
  
  // Handle closing the task detail dialog
  const handleCloseTaskDetail = () => {
    console.log("Closing task detail dialog");
    
    // If there's a selected task, make sure any changes are saved
    if (selectedTask) {
      console.log("Saving task before closing:", selectedTask);
      
      // Force a final update with the current state of the task
